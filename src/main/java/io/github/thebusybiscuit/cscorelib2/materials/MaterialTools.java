...
public final class MaterialTools {
    ...
    static {
        ...
        shovel.addAll(Arrays.asList(Material.SNOW, Material.SNOW_BLOCK, Material.FARMLAND, Material.SOUL_SAND, Material.CLAY, Material.GRAVEL, Material.GRASS_PATH, Material.DIRT, Material.COARSE_DIRT, Material.GRASS_BLOCK, Material.MYCELIUM, Material.PODZOL));

        for (Material mat : Material.values()) {
            if (Tag.SAND.isTagged(mat))
                shovel.add(mat);
            if (mat.toString().endsWith("_CONCRETE_POWDER"))
                shovel.add(mat);
        }
