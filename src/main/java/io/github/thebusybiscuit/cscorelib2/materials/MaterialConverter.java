...
public final class MaterialConverter {
    ...
    public static Optional<Material> getSaplingFromLog(@NonNull Material log) {
        ...
        return convert(log, MaterialCollections.getAllLogs()::contains, type -> type.substring(0, type.lastIndexOf('_')).replace("STRIPPED_", "") + "_SAPLING");
    }

    public static Optional<Material> getSaplingFromLeaves(@NonNull Material leaves) {
        return convert(leaves, MaterialCollections.getAllLeaves()::contains, type -> type.replace("_LEAVES", "_SAPLING"));
    }

    public static Optional<Material> getPlanksFromLog(@NonNull Material log) {
        return convert(log, MaterialCollections.getAllLogs()::contains, type -> type.substring(0, type.lastIndexOf('_')).replace("STRIPPED_", "") + "_PLANKS");
    }

    public static Optional<Material> getStrippedFromLog(@NonNull Material log) {
        return convert(log, type -> MaterialCollections.getAllLogs().contains(type) && !type.name().startsWith("STRIPPED_"), type -> "STRIPPED_" + type);
    }

    public static Optional<Material> getGlassFromPane(@NonNull Material pane) {
        return convert(pane, MaterialCollections.getAllStainedGlassPaneColors()::contains, type -> type.substring(0, type.length() - "_PANE".length()));
    }

    public static Optional<Material> getPaneFromGlass(@NonNull Material glass) {
        return convert(glass, MaterialCollections.getAllStainedGlassColors()::contains, type -> type + "_PANE");
    }

    public static Optional<Material> getWoolFromDye(@NonNull Material dye) {
        return convert(dye, MaterialCollections.getAllDyeColors()::contains, type -> type.replace("_DYE", "_WOOL"));
    }

    public static Optional<Material> getCarpetFromDye(@NonNull Material dye) {
        return convert(dye, MaterialCollections.getAllDyeColors()::contains, type -> type.replace("_DYE", "_CARPET"));
    }

    public static Optional<Material> getStainedGlassFromDye(@NonNull Material dye) {
        return convert(dye, MaterialCollections.getAllDyeColors()::contains, type -> type.replace("_DYE", "_STAINED_GLASS"));
    }

    public static Optional<Material> getStainedGlassPaneFromDye(@NonNull Material dye) {
        return convert(dye, MaterialCollections.getAllDyeColors()::contains, type -> type.replace("_DYE", "_STAINED_GLASS_PANE"));
    }
